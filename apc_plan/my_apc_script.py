...
RECV_INFILL_NAME    = "design_r_blockd_infill"

DEFAULT_LINE_FIELD  = "Line_name"
DEFAULT_SQLITE_PATH = r"E:/25 BIRBA/01-DataBase/APC_Plan_Database_BB.sqlite3"
APP_TITLE = "APC Swath Filter"

# Block list: (UI text, layer suffix, DB key)
#   - Layers: design_s_<suffix>, design_r_<suffix>, design_r_<suffix>_infill
#   - DB tables: <db_key>_normal, <db_key>_infill
BLOCK_OPTIONS = [
...
class SwathFilterDock(QtWidgets.QDockWidget):
    def __init__(self, parent=None):
        ...
        try:
            ...
            for text, layer_suffix, db_key in BLOCK_OPTIONS:
                # Store (layer_suffix, db_key) as userData
                self.blockCombo.addItem(text, (layer_suffix, db_key))
            # Default to last option (Block D)
            ...
            # Immediately hide debug buttons so they don't appear
            self.testBtn.hide()
            self.pingBtn.hide()
            ...
            self.sqliteLabel = QtWidgets.QLabel("SQLite")   # <<< label we can hide/show
            ...
            # test/ping are hidden, but keeping them in layout is okay too:
            ...
            self.signatureLbl.setStyleSheet(
                "QLabel { color: #666666; font-size: 10pt; }"
            )
    ...
    def _get_block_keys(self):
        """
        Returns (layer_suffix, db_key) for current block.
        Example: ('BlockD', 'blockd')
        """
        data = self.blockCombo.currentData()
        if data:
            return data
        # Fallback safely to Block D if something goes wrong
        return ("BlockD", "blockd")

    def _get_layer_names_for_current_block(self):
        """
        Returns (src_name, rcv_normal_name, rcv_infill_name) for the selected block.
        Example: ("design_s_blockd", "design_r_blockd", "design_r_blockd_infill")
        """
        layer_suffix, _db_key = self._get_block_keys()
        layer_suffix = layer_suffix.lower()  # match your lowercase layer names

        src_name  = f"design_s_{layer_suffix}"
        rcv_norm  = f"design_r_{layer_suffix}"
        rcv_infil = f"design_r_{layer_suffix}_infill"
        return src_name, rcv_norm, rcv_infil
    ...
    def _toggle_db_frames(self, *args):
        try:
            if self.dbtype.currentText() == "postgresql":
                self.pgBox.show()
                self.sqliteRow.hide()
                self.sqliteLabel.hide()   # <<< hide label too
            else:
                self.pgBox.hide()
                self.sqliteRow.show()
                self.sqliteLabel.show()   # <<< show label again
    ...
    def test_everything(self):
        try:
            # Determine layer names for current block
            src_name, rcv_norm_name, rcv_infill_name = self._get_layer_names_for_current_block()

            lyr_s  = get_layer_by_name(src_name)
            lyr_rn = get_layer_by_name(rcv_norm_name)
            lyr_ri = get_layer_by_name(rcv_infill_name)

            missing = [
                n for n, l in [
                    (src_name,       lyr_s),
                    (rcv_norm_name,  lyr_rn),
                    (rcv_infill_name,lyr_ri),
                ]
                if l is None
            ]
            if missing:
            ...
            ok2 = table_exists(conn, dbt, f"{blk_db}_infill")
            conn.close()
            if not ok1 and not ok2:
                warn(f"Tables not found: {blk_db}_normal, {blk_db}_infill")
            else:
    ...
    def apply(self):
        try:
            layer_suffix, blk_db = self._get_block_keys()
            src_name, rcv_norm_name, rcv_infill_name = self._get_layer_names_for_current_block()
            ...
            # ✅ FIX: make swath range work in both directions (max→min OR min→max)
            sw_lo, sw_hi = (a, b) if a <= b else (b, a)

            conn, dbt = self._get_conn()

            # Fetch ranges to *display* and to base counts on
            src_from, src_to, rcvN_from, rcvN_to, rcvI_from, rcvI_to = fetch_line_ranges_for_range(
            ...
            # Actual counts after filter (kept as before, but safe)
            actual_vps_total   = cnt_s
            normal_nodes_total = cnt_rn
            infill_nodes_total = cnt_ri
            ...
            self.status.setText(
                ...
                f"Number of Infill Receiver Lines: "
                f"{infill_receiver_lines if infill_receiver_lines is not None else ''}\n\n"
                f"Number of Normal Nodes: {_fmt_count(normal_nodes_total)}\n\n"
                f"Number of Infill Nodes: {_fmt_count(infill_nodes_total)}\n\n"
                f"Design VPs: {design_vps_total if design_vps_total is not None else ''}\n"
                #f"Actual VPs: {_fmt_count(actual_vps_total)}\n"
            )
    ...
    def clear_filters(self):
        try:
            src_name, rcv_norm_name, rcv_infill_name = self._get_layer_names_for_current_block()
            for nm in (src_name, rcv_norm_name, rcv_infill_name):
...
def main(qgis_iface=None):
    ...
    if qgis_iface is not None:
        # if you want to override global iface, you can do it here
        pass
