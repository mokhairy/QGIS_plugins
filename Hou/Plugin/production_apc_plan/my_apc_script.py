...
APP_TITLE = "Production APC Swath Filter"
...
class SwathFilterDock(QtWidgets.QDockWidget):
    def __init__(self, parent=None):
        ...
        try:
            ...
            self.pgDb   = QtWidgets.QLineEdit("Production_APC")  # <— set to Production_APC
...
def main(qgis_iface=None):
    ...
    """
    # if a custom iface is passed (from plugin), we can use it; otherwise use global iface
    if qgis_iface is not None:
        # If you want to prefer injected iface, you can assign it globally here if needed
        pass
    show_apc_swath_filter()
    return f"APC Swath Filter loaded • {SIGNATURE_TEXT}"

# Hussein Al Shibli  |  Production APC  |  BGP Oman Crew 8622
