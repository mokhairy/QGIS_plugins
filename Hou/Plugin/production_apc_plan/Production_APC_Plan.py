# -*- coding: utf-8 -*-
"""
Production APC Plan – QGIS 3 Plugin
...
# from .Production_APC_Plan_dialog import ProductionAPCDialog
...
def _log(msg: str, level=Qgis.Info):
    try:
        iface.messageBar().pushMessage("Production APC Plan", str(msg), level=level, duration=6)
    except Exception:
        pass
    QgsMessageLog.logMessage(str(msg), "Production APC Plan", level)
...
class ProductionAPC:
    def __init__(self, iface_):
        ...
        self.actions = []
        self.menu = self.tr("&Production APC Plan")

    def tr(self, message):
        return QCoreApplication.translate("ProductionAPC", message)
    ...
    def initGui(self):
        ...
        self.toolbar = self.iface.addToolBar("Production APC")
        self.toolbar.setObjectName("ProductionAPCToolbar")

        icon_path = ':/plugins/Production_APC_Plan/icons/hou_pdo_red_padded.svg'
        action = QAction(QIcon(icon_path), "Production APC", self.iface.mainWindow())
        ...
        action.setToolTip("Production APC Tool")
        self.toolbar.setIconSize(QtCore.QSize(32, 32))  # <— add this line!

        self.toolbar.addAction(action)
        self.iface.addPluginToMenu("&Production APC Plan", action)
    ...
    def run(self):
        ...
        try:
            ...
            QMessageBox.critical(self.iface.mainWindow(), "Production APC Plan", f"Error: {e}")
